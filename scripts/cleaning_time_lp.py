...
def optimise_cleaning_times(
    ...
    for shift in shifts:
        ...
        rhs = gp.LinExpr()
        rhs += slack_time_vars[shift]
        rhs -= overtime_vars[shift]
    ...
    for shift in shifts:
        ...
        cleaning_time = shift_info[shift]["n_stops"] * round(optimised_times["setup_time"])

        cleaning_time += shift_info[shift]["n_stops_by_type"]["Tramhalte"][0] * round(optimised_times["tram_time"])
        cleaning_time += shift_info[shift]["n_stops_by_type"]["Tramhalte"][1] * round(optimised_times["tram_abri_time"])
...
def total_travel_time(distances: dict, locations: list, penalty: float) -> float:
