...
public class SolveBalancedSA {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                ...
                // Balanced obj
                // [0] = sumL, [1] = sumSquaredL, [2] = sumC, [3] = sumSquaredC
                double[] sums = new double[4];
                for (Shift shift : shifts) {
                    double L = shift.totalTime;
                    double C = shift.serviceTime;
                ...
                // Use the balanced objective function for costs
                double GreedyF = objFunction(sums[0], sums[1], sums[2], sums[3], shifts.size());
                System.out.println("Balanced objective: " + GreedyF / 60.0 + " hours.");

                // Do the SA
                System.out.println("\nSolving SA:");
                long saStartRunTime = System.currentTimeMillis();
                List<Shift> saShifts = solveBalancedSA(shifts, allStops, travelTimes);
                ...
                // Use the balanced objective function for costs
                double SAF = objFunction(sumsSA[0], sumsSA[1], sumsSA[2], sumsSA[3], saShifts.size());
                System.out.println("Balanced objective: " + SAF / 60.0 + " hours.");
    ...
    /**
     * Method that does the actual simulated annealing algorithm with a balanced objective function
     *
     ...
     */
    public static List<Shift> solveBalancedSA(List<Shift> initialShifts, HTMInstance instance, double[][] travelTimes) {
        ...
        // Sums for the penalty
        int m = currentBest.size();
        ...
        // Use the balanced objective function for costs
        double currentF = objFunction(sums[0], sums[1], sums[2], sums[3], m);
        double bestF = currentF;

        // Keep track of best shift found
        ...
        while (System.currentTimeMillis() - startSA < timeLimit) {
            ...
            // Accept / reject decision using balanced objective function
            int shift1 = shiftIdOut[0];
            int shift2 = shiftIdOut[1];

            // Copy the current sums
            double[] currentSums = new double[] {
                    sums[0], sums[1], sums[2], sums[3]
            };

            // Update the sums based on the changed shifts
            if (shift1 != -1) {
                Shift oldShift = currentBest.get(shift1);
                Shift newShift = neighbour.get(shift1);
                updateSums(currentSums, oldShift.totalTime, newShift.totalTime, oldShift.serviceTime, newShift.serviceTime);
            }
            if (shift2 != -1) {
                Shift oldShift = currentBest.get(shift2);
                Shift newShift = neighbour.get(shift2);
                updateSums(currentSums, oldShift.totalTime, newShift.totalTime, oldShift.serviceTime, newShift.serviceTime);
            }

            // Calculate objective after the move
            double newF = objFunction(currentSums[0],currentSums[1], currentSums[2], currentSums[3], m);

            // Check if objective changed with this move
            if (Math.abs(newF - currentF) < 1e-9) {
                unchanged++;
            }
            ...
            if (newF < currentF) {
                ...
                double P = Math.exp((currentF - newF) / T);
                if (r < P) {
                    currentBest = neighbour;
                    sums = currentSums;
                    currentF = newF;
    ...
    public static Shift findMinLengthShift(List<Shift> shifts) {
        double minLength = 10000000.0;
        Shift minShift = null;
        for (Shift shift : shifts) {
            if (shift.totalTime < minLength) {
                minLength = shift.totalTime;
                minShift = shift;
            }
        }
        return minShift;
    ...
    public static Shift findMinCleaningShift(List<Shift> shifts) {
        double minClean = 10000000.0;
        Shift minCleanShift = null;
        for (Shift shift : shifts) {
            if (shift.serviceTime < minClean) {
                minClean = shift.serviceTime;
                minCleanShift = shift;
            }
        }
        return minCleanShift;
    }


    /**
     * Calculates objective based on a balanced objective function that gives a penalty to uneven shift lengths and
     * cleaning times
     *
     * @param sumL total length of all shifts
     * @param sumSquaredL total length of all shifts squared
     * @param sumC total cleaning time of all shifts
     * @param sumSquaredC total cleaning time of all shifts squared
     * @param m total number of shifts
     * @return the objective
     */
    private static double objFunction(double sumL, double sumSquaredL, double sumC, double sumSquaredC, int m) {
        double sseL = sumSquaredL - (sumL * sumL) / m;
        double sseC = sumSquaredC - (sumC * sumC) / m;

        return sumL + lambdaL * sseL + lambdaC * sseC;
    }

    /**
     * Updates the sums so that no full recalculation is necessary
     *
     * @param sums array of sums: [0] = sumL, [1] = sumSquaredL, [2] = sumC, [3] = sumSquaredC
     * @param oldL old L
     * @param newL new L
     * @param oldC old C
     * @param newC new C
     */
    private static void updateSums(double[] sums, double oldL, double newL, double oldC, double newC) {
        sums[0] += (newL - oldL);
        sums[1] += (newL * newL - oldL * oldL);
        sums[2] += (newC - oldC);
        sums[3] += (newC * newC - oldC * oldC);
    }
